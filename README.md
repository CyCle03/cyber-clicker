...
열람권(내 데이터 내려받기) 문서는 **한국어·영어 두 벌**이다. 통합 인증이 내부 호출 본문에 `lang` 을 실어 보내고, `en` 이면 영어 키로 된 문서를 낸다(모르는 값이면 한국어). 키를 그때그때 번역하지 않는 건 이게 받아서 보관하는 파일이라, 같은 키가 언어에 따라 달라지면 이미 받아 둔 파일과 비교가 안 되기 때문이다.
